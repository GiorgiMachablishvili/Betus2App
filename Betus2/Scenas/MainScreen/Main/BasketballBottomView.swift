...
class BasketballBottomView: UIView {

    private lazy var basketballBottomViewTitle: UILabel = {
        let view = UILabel(frame: .zero)
        view.text = "Daily basketball workout"
    ...
    private lazy var basketballWorkoutDescription: UILabel = {
        let view = UILabel(frame: .zero)
        view.text = "Daily basketball training: improve your coordination, endurance and skill in the game. Develop reaction, speed and flexibility on the court with every exercise."
    ...
    private lazy var basketballTimeLabel: UILabel = {
        let view = UILabel(frame: .zero)
        view.text = "30 min"
        view.textColor = UIColor(hexString: "#FFFFFF")
        view.font = UIFont.goldmanBold(size: 14)
    ...
    private func setup() {
        addSubview(basketballBottomViewTitle)
        addSubview(basketballWorkoutDescription)
        addSubview(timerImage)
        addSubview(basketballTimeLabel)
        addSubview(startButton)
    }

    private func setupConstraints() {
        basketballBottomViewTitle.snp.remakeConstraints { make in
            make.top.equalTo(snp.top).offset(16)
            make.leading.equalTo(snp.leading).offset(16)
            make.height.equalTo(17)
        }

        basketballWorkoutDescription.snp.remakeConstraints { make in
            make.top.equalTo(basketballBottomViewTitle.snp.bottom).offset(8)
            make.leading.equalTo(snp.leading).offset(16)
        ...
        basketballTimeLabel.snp.remakeConstraints { make in
